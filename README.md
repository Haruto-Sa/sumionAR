...
Hiro マーカーと位置情報ベース AR（LocAR.js）を最小構成でまとめたプロジェクトです。

- **マーカー AR**: Hiro マーカー上に Duck / Suimon / Wankosoba の GLB モデルを表示  
- **固定地点 AR**: `config/locations.yaml` で定義した地点に 3D モデルを配置し、高さ・サイズ・向きをブラウザから調整  
...
│   └── models.yaml         # 使用する 3D モデル定義（GLB 名など、将来拡張用）
├── src/
│   ├── location/
│   │   ├── core.ts         # LocAR.js を使った共通 3D シーン制御
│   │   └── uiToggle.ts     # UI 最小化ボタン
│   └── location-ar/
│       └── main.ts         # 固定地点 AR のメインロジック
├── server.py               # 任意: ログ付き Python HTTP サーバー（未使用でも可）
├── package.json            # Vite + TypeScript + locar の設定
├── vite.config.mjs         # Vite 設定（input: index/marker-ar/location-ar, base: /sumionAR/）
└── .gitignore              # doc/manual/, node_modules/, dist/ などを除外
```
...
- カメラと位置情報の許可を与えると、`config/locations.yaml` に定義された地点まわりに 3D モデルが配置されます。
...
現状のロジックでは直接は参照していませんが、モデル一覧の管理や将来拡張用に利用できます。

## トップページのマップ（OpenStreetMap）

`index.html` の下部に、Leaflet + OpenStreetMap を用いた簡易マップを表示しています。

- `config/locations.yaml` の `locations` を読み込んでマーカー表示
- 各地点は `icon`（絵文字） + `name` のポップアップで表示
- 地点が 1 件以上あれば `fitBounds` で全地点が入るよう自動調整

## ライセンス / 用途

このリポジトリは、Hiro マーカー AR と固定地点 AR（LocAR.js ベース）の検証・学習用コードです。  
再利用や商用利用の際は、依存ライブラリ（AR.js, A-Frame, LocAR.js, three.js 等）のライセンスも併せて確認してください。
